...
export async function GET() {
  ...
  const results = videographers.map((v) => {
    ...
    for (const b of bookings) {
      ...
      for (const s of b.services) {
        if      (s.serviceType === "VIDEO_STANDARD") standardCount++
          ...
          photoCount++
          simPhotoTotal += PHOTO_RATES[s.serviceType] ?? CUR_PHOTO_RATE
        }
      }
    }
    ...
    // Simulated model (no base salary, new rates)
  ...
  return NextResponse.json({
    ...
    rates: {
      standard:   NEW_STANDARD_RATE,
      drone:      NEW_DRONE_RATE,
      photoRates: PHOTO_RATES,
      intro:      INTRO_RATE,
    },
    revenue: {
