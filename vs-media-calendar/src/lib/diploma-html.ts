export type DiplomaType = "video" | "intros" | "spender"

export interface DiplomaWinner {
  name: string
  image: string
}
...
.stripe{height:5px;background:linear-gradient(90deg,transparent 0%,${cfg.accent} 50%,transparent 100%)}
.body{padding:40px 48px;position:relative;overflow:hidden}
.ring1{position:absolute;pointer-events:none;top:-60px;right:-60px;width:220px;height:220px;border-radius:50%;border:1px solid rgba(${cfg.accentRgb},.13)}
.ring2{position:absolute;pointer-events:none;bottom:-80px;left:-60px;width:280px;height:280px;border-radius:50%;border:1px solid rgba(${cfg.accentRgb},.10)}
...
.icon-wrap{display:inline-flex;align-items:center;justify-content:center;width:68px;height:68px;border-radius:18px;background:rgba(${cfg.accentRgb},.12);border:1px solid rgba(${cfg.accentRgb},.25);font-size:34px;margin-bottom:14px}
...
.avatar,.avatar-ph{width:88px;height:88px;border-radius:50%;border:4px solid rgba(${cfg.accentRgb},.4);margin:0 auto 14px}
.avatar-sm,.avatar-ph-sm{width:68px;height:68px;border-radius:50%;border:3px solid rgba(${cfg.accentRgb},.4)}
.avatar,.avatar-sm{display:block;object-fit:cover}
.avatar-ph{display:flex;align-items:center;justify-content:center;background:rgba(${cfg.accentRgb},.1);font-size:34px;font-weight:900;color:${cfg.accent}}
.avatar-ph-sm{display:flex;align-items:center;justify-content:center;background:rgba(${cfg.accentRgb},.1);font-size:26px;font-weight:900;color:${cfg.accent}}
...
.metric-box{text-align:center;border-radius:18px;border:1px solid rgba(${cfg.accentRgb},.22);background:rgba(${cfg.accentRgb},.09);padding:20px 24px;margin-bottom:28px;position:relative;z-index:1}
...
.footer{border-top:1px solid rgba(255,255,255,.07);padding-top:18px;display:flex;justify-content:space-between;align-items:center;position:relative;z-index:1}
.dots{display:flex;gap:4px}
.dot{width:5px;height:5px;border-radius:50%;background:${cfg.accent}}
.actions{display:flex;gap:12px}
.btn{padding:10px 24px;border-radius:12px;font-size:13px;font-weight:600;cursor:pointer;border:none;font-family:inherit}
...
<div class="diploma">
  ...
  <div class="body">
    ...
    <div class="ring2"></div>
    <div class="header">
      <svg class="brand" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 476.51 68.49">${LOGO_SVG.replace(/^<svg[^>]*>/, "").replace(/<\/svg>$/, "")}</svg>
    ...
    <div class="footer">
      ${LOGO_SVG.replace("<svg ", `<svg style="height:10px;width:auto;opacity:.22" `)}
      <div class="dots">
